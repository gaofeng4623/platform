  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
            ...
            function fetchModel(modelId) {
                ...
                var modelUrl = KISBPM.URL.getModel(modelId)+"?"+Math.random();
                $http({method: 'GET',cache: false, url: modelUrl, headers: {'Pragma': 'no-cache','Cache-Control':'no-cache'}}).
    ...
    activitiModeler.directive('assigneetree', function () {
	    return {  
	        require: '?ngModel',  
	        restrict: 'A',  
	        link: function ($scope, element, attrs, ngModel) { 
	        	var setting = {
        			async:{  
        		        enable:true,  
        		        type:"post",  
        		        url: baseUrl+"/workflow/model/assignee",
        		        otherParam: {
        				...
        				onCheck: function (event, treeId, treeNode) {  
	                     ...
	                     onAsyncSuccess: function(event, treeId, treeNode) {
				            ...
				            if (assignee && "" != assignee) {
				            	 console.log(assignee);
					            var node = tree.getNodeByParam("id", assignee);
					            console.log(node);
	            ...
	            jQuery.fn.zTree.init(element, setting);
	        }  
	    };  
	});
	activitiModeler.directive('candidateuserstree', function () {
	    return {  
	        require: '?ngModel',  
	        restrict: 'A',  
	        link: function ($scope, element, attrs, ngModel) {
	        	var setting = {
        			async:{  
        		        enable:true,  
        		        type:"post",  
        		        url: baseUrl+"/workflow/model/candidateUsers"
        		    },  
        			check: {
        				enable: true
        			},
        			...
        			callback: {  
        				onCheck: function (event, treeId, treeNode) {
        					...
        					if (nodes && nodes.length>0) {
        						...
        						$scope.$apply(function () {  
        							ngModel.$setViewValue([{value: ids.join(',')}]); 
        						}); 
        					}
                         },
                         onAsyncSuccess: function(event, treeId, treeNode) {
				        	...
				        	var values = $scope.assignment.candidateUsers;
				        	for (var i = 0; i <values.length; i++) {
				        		candidateUsers.push(values[i].value);
				        	}
				        	if (candidateUsers.length > 0) {
				        		var ids = candidateUsers.join(',').split(',');
	...
	activitiModeler.directive('candidategroupstree', function () {
	    return {  
	        require: '?ngModel',  
	        restrict: 'A',  
	        link: function ($scope, element, attrs, ngModel) { 
	        	...
	        	var setting = {
        			async:{
        		        enable:true,  
        		        type:"post",  
        		        url: baseUrl+"/workflow/model/candidateGroups"
        		    },  
        			check: {
        				enable: true
        			},
        			...
        			callback: {  
        				onCheck: function (event, treeId, treeNode) {  
                         ...
                         onAsyncSuccess: function(event, treeId, treeNode) {
                     	 	var tree = jQuery.fn.zTree.getZTreeObj(treeId);
                     	 	var candidateGroups = new Array();
				        	var values = $scope.assignment.candidateGroups;
				        	for (var i = 0; i <values.length; i++) {
				        		candidateGroups.push(values[i].value);
				        	}
				        	if (candidateGroups.length > 0) {
				        		var ids = candidateGroups.join(',').split(',');
					        	for (var i = 0; i <ids.length; i++) {
									var node = tree.getNodeByParam("id", ids[i]);
									try{tree.checkNode(node, true, false);}catch(e){}
					        	}
