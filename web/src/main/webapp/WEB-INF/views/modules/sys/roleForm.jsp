		...
		$(document).ready(function(){
			...
			$("#inputForm").validate({
				rules: {
					name: {
						remote : {
							url: "${ctx}/sys/role/checkName?id=${role.id}",
							data : {
								'office.id' : function() {
									return $('#officeId').val()
								}
					...
					enname: {remote: "${ctx}/sys/role/checkEnname?id=${role.id}"}
				},
				messages: {
					name: {remote: "同级组织机构下角色名已存在"},
				...
				submitHandler: function(form){
					...
					var statiIds = [], statiNodes = treeStati.getCheckedNodes(true);
					for(var i=0; i<statiNodes.length; i++) {
						if ("" != statiNodes[i].id) {
							statiIds.push(statiNodes[i].id);
						}
					}
					$("#statiIds").val(statiIds);
			...
			var settingOffice = {check:{enable:true,nocheckInherit:false,chkboxType:{"Y" : "", "N" : "" }},view:{selectedMulti:false},
					data:{simpleData:{enable:true}},callback:{beforeClick:function(id, node){
						tree.checkNode(node, !node.checked, true, true);
						return false;
					}}};
			...
			// 初始化树结构
			var tree2 = $.fn.zTree.init($("#officeTree"), settingOffice, zNodes2);
			// 不选择父节点
			...
			// 统计信息
			var zNodesStati=[
					<c:forEach items="${statiList}" var="stati">{id:"${stati.statiKey}", pId:"${stati.pId}", name:"${stati.statiName}", nocheck: ${stati.nocheck}},
		            </c:forEach>];
			var treeStati = $.fn.zTree.init($("#statiTree"), setting, zNodesStati);
			var statiIdsArr = "${statiIds}".split(",");
			for(var i=0; i<statiIdsArr.length; i++) {
				var node = treeStati.getNodeByParam("id", statiIdsArr[i]);
				try{treeStati.checkNode(node, true, false);}catch(e){}
			}
			treeStati.expandAll(true);
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/sys/role/list?office.id=${role.office.id}&office.name=${role.office.name}">角色列表</a></li>
		<li class="active"><a href="${ctx}/sys/role/form?id=${role.id}&office.id=${role.office.id}&office.name=${role.office.name}">角色<shiro:hasPermission name="sys:role:edit">${not empty role.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:role:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="role" action="${ctx}/sys/role/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="office" name="office.id" value="${role.office.id}" labelName="office.name" labelValue="${role.office.name}"
					title="机构" url="/sys/office/treeData" cssClass="required" dataMsgRequired="必填信息"/>
		...
		<!-- 	<label class="control-label"></label> -->
			<div class="controls">
					...
					<div class="span4">
						首页统计  ↓
						<div id="statiTree" class="ztree" ></div>
						<input type="hidden" name="statiIds" id="statiIds" value="${statiIds}"/>
					</div>
					<div class="span4">
						数据授权  ↓
