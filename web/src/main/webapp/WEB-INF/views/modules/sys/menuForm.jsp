	...
	<form:form id="inputForm" modelAttribute="menu" action="${ctx}/sys/menu/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<select name="isShow" class="input-xlarge required">
					<option value="">请选择</option>
					<c:forEach items="${fns:getDictList('show_hide')}" var="item">
						<option value="${item.value}" <c:if test="${item.value==menu.isShow}"> selected=selected</c:if> >${item.label}</option>
					</c:forEach>
				</select>
