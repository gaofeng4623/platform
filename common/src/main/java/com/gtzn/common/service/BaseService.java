	...
	public static String dataScopeFilter(User user, String officeAlias, String userAlias) {
		...
		if (!user.isAdmin()) {
			...
			for (Role role : user.getRoleList()){
				for (String office : StringUtils.split(officeAlias, ",")) {
					if (!dataScope.contains(role.getDataScope()) && StringUtils.isNotBlank(office)) {
						//1：所有数据
						...
						//2：所在公司及以下数据
							...
							sqlString.append(" OR " + office + ".id = '" + user.getCompany().getId() + "'");
							sqlString.append(" OR " + office + ".parent_ids LIKE '" + user.getCompany().getParentIds() + user.getCompany().getId() + ",%'");
						}
						//3：所在公司数据；
						else if (Role.DATA_SCOPE_COMPANY.equals(role.getDataScope())) {
							sqlString.append(" OR " + office + ".id = '" + user.getCompany().getId() + "'");
							// 包括本公司下的部门 （type=1:公司；type=2：部门）
							sqlString.append(" OR (" + office + ".parent_id = '" + user.getCompany().getId() + "' AND " + office + ".type = '2')");
						}
						//4：所在部门及以下数据
						else if (Role.DATA_SCOPE_OFFICE_AND_CHILD.equals(role.getDataScope())) {
							sqlString.append(" OR " + office + ".id = '" + user.getOffice().getId() + "'");
							sqlString.append(" OR " + office + ".parent_ids LIKE '" + user.getOffice().getParentIds() + user.getOffice().getId() + ",%'");
						}
						//5：所在部门数据
...
//							}
							sqlString.append(" OR EXISTS (SELECT 1 FROM sys_role_office WHERE role_id = '" + role.getId() + "'");
							sqlString.append(" AND office_id = " + office +".id)");
						}
						//8：仅本人数据
...
//		System.out.println("dataScopeFilter: " + sqlString.toString());
	...
	protected  <T extends DataEntity<T>> Pager<T> findPage(CrudDao<T> dao, Pager<T> pager, T entity, User user) {
		entity.setPager(pager);
		entity.getSqlMap().put("dsf", dataScopeFilter(user, "o", ""));
